...
async def created_by_user(db: AsyncSession, article_id: str, user_id: str):
    results = await ArticleRepository(db).filter_by_spec(ArticleSpecification.created_by_user(user_id) &
                                       ArticleSpecification.id_is(article_id) & ArticleSpecification.not_deleted())
    return len(results)
...
async def get_article_by_id(db, article_id):
    articles = await ArticleRepository(db).filter_by_spec(spec=ArticleSpecification.not_deleted() & ArticleSpecification.id_is(id=article_id))
    if not articles:
        raise HTTPException(status_code=404, detail="Not found")
