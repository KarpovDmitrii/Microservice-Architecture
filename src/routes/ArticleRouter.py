...
async def get_article_by_id(id: str, db = Depends(get_db)):
    answer = await ArticleController.get_article_by_id(db, id)
    return answer

@ArticleRouter.put("/articles/{article_id}", tags=["articles"])
async def update_article_data(updated_article: CreateArticleSchema, id: str , token_data=Depends(access_token_required), db = Depends(get_db)):
    answer = await ArticleController.update_article_data(db, id, updated_article, token_data)
    return answer
